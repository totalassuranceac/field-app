-- Close path when a drop-off was not a real warranty (e.g. part going out to another job).
-- Closed statuses: approved | rejected | not_warranty
-- Open statuses unchanged: dropped_off, claim_submitted, return_to_vendor, delivered
...
CREATE TABLE IF NOT EXISTS warranty_claims_new (
  ...
  shipped_by_user_id INTEGER REFERENCES users(id)
);
...
INSERT INTO warranty_claims_new (
  ...
  rma_number, credit_amount, tracking_number, shipped_by_user_id
)
SELECT
  id,
  log_number,
  CASE status
    ...
    WHEN 'not_warranty' THEN 'not_warranty'
    WHEN 'claim_submitted' THEN 'claim_submitted'
    WHEN 'return_to_vendor' THEN 'return_to_vendor'
