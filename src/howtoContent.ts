...
export const HOWTO_GUIDES: HowToGuide[] = [
  ...
  },
  {
    ...
    tips: [
      ...
      "While a claim is open, warehouse uses Status notes so everyone can see progress. Do not scrap Hold parts — vendors sometimes require a return for credit.",
    ],
  },
  {
  ...
  },
  {
    ...
    summary: "Shop piles: File · Hold · Return · Parked. Match the shelf piles.",
    ...
    steps: [
      ...
      "If the vendor emails send-it-back for credit (e.g. Crestmont / ACES), tap Return to vendor — do not throw it away.",
      "Johnstone and Solar Supply drop-offs land on Return (not File). Never scrap Solar.",
      "Solar: when they have the failed part and delete the invoice, tap “Solar received return — invoice deleted” (Approved $0). Johnstone is counter return only — no that button.",
      "Scrap only after the vendor says dispose, or credit posts with no return ask.",
      "Park = do not file / do not scrap (e.g. new purchase) — requires a why-note; Unpark when ready.",
      "Stop if buy-invoice parts do not match the card / nameplate.",
    ],
    tips: [
      "Submitted is not done. Do not scrap on Claim submitted / Hold.",
      "Don’t know the vendor? Inventory → Pickup → “Where did we buy this?” by address.",
    ],
  },
  {
    id: "warranty-piles",
    title: "Warranty piles (File / Hold / Return)",
    summary: "How shop piles map to the app — Solar closeout is invoice deleted.",
    roles: ["warehouse", "admin", "office", "mechanic", "everyone"],
    path: "/warranties",
    steps: [
      "File pile: parts waiting for a claim to be filed (unsubmitted, not parked).",
      "Hold: claim submitted, waiting on the vendor — do NOT scrap (return requests can still come in).",
      "Return: Johnstone, Solar Supply, or vendor said send it back for credit — never scrap; never file online.",
      "Solar Supply: replacement free; when Solar receives the failed part they delete the invoice ($0 closeout). Never scrap Solar.",
      "Johnstone: return to the counter only — no invoice-deleted button.",
      "Parked: new-purchase / do-not-file — off the File badge until Unpark (why-note required).",
      "Scrap only after vendor says dispose, or credit posts with no return ask.",
      "If the purchase invoice parts do not match the nameplate/card, stop and ask Chris.",
    ],
  },
  {
    id: "warranty-vendor-filing",
    title: "Warranty filing by vendor",
    summary: "ACES is email — Goodman/Daikin/Lennox/Ferguson use their portals. Do not mix them up.",
    roles: ["warehouse", "admin", "office", "mechanic"],
    path: "/warranties",
    steps: [
      "ACES / American Standard: official warranty form → email Victoria. NEVER the portal.",
      "Goodman / Daikin: Warranty Express portal (not email-a-form).",
      "Lennox: LennoxPros.",
      "Ferguson: Ferguson.com.",
      "Johnstone / Solar Supply: Return pile in the app — physical return, not an online file.",
      "After you file (or start the return), mark Claim submitted or Return to vendor so the piles match the shop.",
    ],
    tips: [
      "Do not lump Goodman with ACES. ACES = email Victoria. Goodman/Daikin = Warranty Express.",
    ],
  },
  ...
  },
  {
    ...
    summary: "Hold = waiting on vendor (do not scrap). Return = send-back for credit.",
    roles: ["warehouse", "admin", "office", "mechanic"],
    path: "/warranties",
    steps: [
      "Hold (Claim submitted) means waiting on the vendor — do NOT scrap; they may still require a return for credit.",
      "If the vendor says return the part (e.g. ACES email on Crestmont), tap Return to vendor so it leaves Hold and sits on Return.",
      "Add RMA # / tracking when you have them. Search works across piles (log #, street, part, RMA).",
      "Solar: “invoice deleted” closeout only — Approved $0. Never scrap Solar. Johnstone = counter return only.",
      "Scrap only after the vendor says dispose, or credit posts with no return ask — then Approved with credit as needed.",
    ],
  },
];
